__kernel void fz_function(__global float *output,__global float *step,__global float *point,int Ny,int Nz,int Ntot) { 
 

    /*LOCAL VARIABLES TO THE WORK-ITEMS*/
    unsigned int indx_x,indx_y,indx_z;
    float x_point,y_point,z_point,target,r,r2,fact;
   ...
   if(global_indx < (unsigned int)Ntot)
   {
	...
	z_point = point[2] + indx_z*step[2];
        ...
        target = exp(-r);
        fact = 2.0*(z_point*z_point*z_point) - (3.0*z_point)*(x_point*x_point + y_point*y_point);
        target = target*fact;
